    ...
    return (
      <section className="card card-hover p-6">
        <div className="flex items-center gap-3 text-[#374151]">
          ...
          <div>
            <h3 className="text-lg font-semibold text-[#111827]">AI 提案を待っています</h3>
            <p className="text-sm text-[#6b7280]">振り返りを保存してから AI 提案を生成することで、次週のフォーカスが明確になります。</p>
          </div>
        </div>
      </section>
    );
      ...
      <section className="card card-hover border border-red-500/40 p-6 bg-red-500/10">
        <div className="flex items-start gap-3 text-red-200">
          ...
          <div className="space-y-2">
            <div>
              <h3 className="text-lg font-semibold text-[#111827]">AI 提案の生成に失敗しました</h3>
  ...
  return (
    <section className="card card-hover p-6 space-y-4">
      <header className="flex items-start justify-between gap-4">
        <div>
          <h3 className="text-xl font-semibold text-[#111827]">AI 提案</h3>
          <p className="text-sm text-[#374151] mt-1">週次のログを解析し、次週に向けたフォーカスと改善案を提案します。</p>
        </div>
        <div className="text-xs text-[#9ca3af] space-y-1 text-right">
          <div>生成: {formatDate(insight.generatedAt, 'yyyy/MM/dd HH:mm')}</div>
          <div className="uppercase tracking-wide text-[#6b7280]">Engine: {insight.engine}</div>
        </div>
      </header>

      <div className="bg-[#f4f5f7]/40 border border-[#e5e7eb] rounded-xl p-5 space-y-4">
        <div>
          <h4 className="text-sm font-semibold text-primary-cyan uppercase tracking-wide">Summary</h4>
          <p className="mt-1 text-sm text-[#374151] leading-relaxed">{insight.summary}</p>
        </div>

        <div>
          <h4 className="text-sm font-semibold text-primary-green uppercase tracking-wide flex items-center gap-2">
            Focus Areas
            <span className="text-xs text-[#6b7280]">{insight.focusAreas.length}件</span>
          </h4>
          <ul className="mt-2 space-y-2 text-sm text-[#374151]">
            {insight.focusAreas.map((item, index) => (
              <li key={index} className="rounded-lg border border-[#e5e7eb] bg-[#f9fafb]/60 px-3 py-2">
                {item}
              </li>
            ))}
          </ul>
        ...
        <div>
          <h4 className="text-sm font-semibold text-primary-green uppercase tracking-wide flex items-center gap-2">
            Recommendations
            <span className="text-xs text-[#6b7280]">{insight.recommendations.length}件</span>
          </h4>
          <ul className="mt-2 space-y-2 text-sm text-[#374151]">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div className="rounded-lg border border-[#e5e7eb] bg-[#f9fafb]/50 p-4">
            <h4 className="text-sm font-semibold text-primary-cyan uppercase tracking-wide">Encouragement</h4>
            <p className="mt-2 text-sm text-[#374151] leading-relaxed">{insight.encouragement}</p>
          </div>
          <div className="rounded-lg border border-[#e5e7eb] bg-[#f9fafb]/50 p-4">
            <h4 className="text-sm font-semibold text-primary-cyan uppercase tracking-wide">Energy Advice</h4>
            <p className="mt-2 text-sm text-[#374151] leading-relaxed">{insight.energyAdvice}</p>
          </div>
        </div>
      </div>

      <footer className="flex items-center justify-between pt-2 text-xs text-[#6b7280]">
