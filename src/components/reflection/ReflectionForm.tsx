  ...
  return (
    <section className="card card-hover p-6 space-y-6">
      <header className="flex items-start justify-between gap-4">
        <div>
          <h3 className="text-xl font-semibold text-[#111827]">週次リフレクション</h3>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <label className="space-y-2">
          <span className="text-sm font-medium text-[#374151] flex items-center justify-between">
            今週の勝因 <span className="text-xs text-[#9ca3af]">{value.wins.length}文字</span>
          </span>
          <textarea
            value={value.wins}
            onChange={handleFieldChange('wins')}
            rows={4}
            className="w-full rounded-lg border border-[#d1d5db] bg-white px-3 py-2 text-sm text-[#111827] placeholder-[#9ca3af] focus:border-primary-cyan focus:outline-none"
            placeholder="例: 高エネルギー時間帯に優先度Sタスクを固定化できた"
          />
        </label>
        <label className="space-y-2">
          <span className="text-sm font-medium text-[#374151] flex items-center justify-between">
            最大の課題 <span className="text-xs text-[#9ca3af]">{value.challenges.length}文字</span>
          </span>
          <textarea
        ...
        <label className="space-y-2">
          <span className="text-sm font-medium text-[#374151] flex items-center justify-between">
            学び・発見 <span className="text-xs text-[#9ca3af]">{value.learnings.length}文字</span>
          </span>
          <textarea
        ...
        <label className="space-y-2">
          <span className="text-sm font-medium text-[#374151] flex items-center justify-between">
            来週のフォーカス <span className="text-xs text-[#9ca3af]">{value.focusNextWeek.length}文字</span>
          </span>
          <textarea
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <label className="space-y-2">
          <span className="text-sm font-medium text-[#374151]">今週の気分</span>
          <select
            value={value.mood}
            onChange={handleSelectChange('mood')}
            className="w-full rounded-lg border border-[#d1d5db] bg-white px-3 py-2 text-sm text-[#111827] focus:border-primary-cyan focus:outline-none"
          >
        ...
        <label className="space-y-2">
          <span className="text-sm font-medium text-[#374151]">エネルギー残量</span>
          <select
            value={value.energy}
        ...
        <label className="space-y-2 md:col-span-1 md:mt-0 mt-4">
          <span className="text-sm font-medium text-[#374151] flex items-center justify-between">
            メモ <span className="text-xs text-[#9ca3af]">{value.notes.length}文字</span>
          </span>
          <textarea
            value={value.notes}
